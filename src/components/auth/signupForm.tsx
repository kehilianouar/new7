  ...
  const handleSignup = async (data: FormData) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        ...
        setServerError('صيغة الإيميل غير صحيحة، تأكد من كتابته بشكل صحيح')
      } else if (err.code === 'auth/weak-password') {
        setServerError('كلمة المرور ضعيفة، يجب أن تكون 6 أحرف على الأقل')
      } else if (err.code === 'auth/operation-not-allowed') {
        setServerError('عملية التسجيل غير مسموحة حالياً، يرجى المحاولة لاحقاً')
      } else {
        setServerError('حصلت مشكلة في التسجيل، جرب تاني. إذا استمرت المشكلة، تأكد من اتصال الإنترنت')
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        setServerError('تم إغلاق نافذة تسجيل جوجل، يرجى المحاولة مرة أخرى')
      } else if (error.code === 'auth/popup-blocked') {
        setServerError('تم حظر نافذة تسجيل جوجل، يرجى السماح بالنوافذ المنبثقة')
      } else if (error.code === 'auth/network-request-failed') {
        setServerError('مشكلة في الاتصال بالإنترنت، يرجى التحقق من اتصالك')
      } else {
        setServerError('فشل تسجيل الحساب بجوجل، يرجى المحاولة مرة أخرى')
      }
    } finally {
      setIsLoading(false)
    }
